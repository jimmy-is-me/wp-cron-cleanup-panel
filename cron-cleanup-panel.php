...
class Cron_Cleanup_Panel {
	...
	private function get_pressure_report(): array {
		...
		$report['autoload_top']   = $wpdb->get_results(
	...
	public function handle_delete_hook_as(): void {
		...
		if ( $table && $hook !== '' ) {
			$wpdb->delete( $table, [ 'hook' => $hook ], [ '%s' ] );
			$deleted = (int) $wpdb->rows_affected;
		} else {
			$deleted = 0;
		}

		$this->redirect( [ 'hook_deleted' => $deleted, 'hook_name' => urlencode( $hook ) ] );
	}
	...
	public function render(): void {
		...
		<?php if ( isset( $_GET['hook_deleted'] ) ) : ?>
			<div class="notice notice-success is-dismissible"><p>
				✅ Hook <code><?php echo esc_html( urldecode( $_GET['hook_name'] ?? '' ) ); ?></code>
				已刪除 <strong><?php echo (int) $_GET['hook_deleted']; ?></strong> 筆 Action Scheduler 任務。
			</p></div>
		<?php endif; ?>
	...
	private function section_as( array $events ): void {
		...
		?>
		<h2 style="margin-top:28px">📋 Action Scheduler (Target Jobs)
			<span style="font-weight:normal;font-size:13px">(<?php echo count( $events ); ?> 筆</span>
		...
		<table class="widefat striped">
			...
			?>
				<tr style="<?php echo $bg; ?>">
					...
					<td><a class="button button-small" onclick="return confirm('Delete this AS job?')"
